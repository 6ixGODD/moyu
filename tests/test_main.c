...
int main(int argc,char** argv){if(argc==3&&!strcmp(argv[1],"--dump-skin"))return dump_skin(argv[2])?0:1;test_state_and_memory();test_mcp_stdio();test_mcp_http();if(failures){fprintf(stderr,"%d test(s) failed\n",failures);return 1;}printf("all tests passed\n");return 0;}
