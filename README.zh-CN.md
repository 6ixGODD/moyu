...
- 两种聊天入口：双击桌宠使用贴在旁边的轻量输入框；从托盘菜单打开 `moyu-chat.exe`，进行完整终端会话。
- 桌面交互：托盘图标、Windows 原生右键菜单、拖放投喂、悬停反馈和拖拽重定位。
- 性格驱动动作：内置 20 类动作，包括漫步、偷偷靠近指针、发呆、偷看、伸懒腰、打哈欠、调皮闪身、害怕和指针驱赶闪避。
...
- 双击桌宠，在它旁边打开像素风快捷输入框。输入框不会越出当前屏幕，会从一行自动长到五行，之后在文本区内滚动；点击圆形勾选按钮发送。
- 右键桌宠打开 Windows 原生菜单。
- 右键托盘图标打开跟随系统明暗主题的 Windows 原生菜单，并从这里进入终端聊天。
...
- [桌宠动画素材制作规范](docs/skin-production-guide.zh-CN.md)
