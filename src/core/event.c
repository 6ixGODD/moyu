...
static void ev_free_fields(internal_event* e) {
    if (e->str)  { moyu_free(e->str);  e->str  = NULL; }
    if (e->str2) { moyu_free(e->str2); e->str2 = NULL; }
}

void event_queue_free(event_queue* q) {
    if (!q || !q->buf) return;
    for (size_t i = 0; i < q->cap; i++) ev_free_fields(&q->buf[i]);
    moyu_free(q->buf);
    q->buf = NULL;
...
void event_push_action(event_queue* q, const char* name, const char* payload) {
    internal_event e; memset(&e, 0, sizeof(e));
