...
bool mcp_discover(mcp_client* c) {
  ...
  platform_http_resp r = platform_http_post_json(
      c->server_url, c->api_key ? c->api_key : "", body, 10000);
  moyu_free(body);
  if (r.err) {
...
char* mcp_call(mcp_client* c,
  ...
  platform_http_resp r = platform_http_post_json(
      c->server_url, c->api_key ? c->api_key : "", body, 30000);
  moyu_free(body);
