...
void log_set_level(log_level lvl) { g_level = lvl; }
...
void log_write(log_level lvl, const char* file, int line, const char* fmt, ...) {
    ...
    localtime_r(&now, &tmv);
#endif
    n += snprintf(buf + n, sizeof(buf) - n, "%04d-%02d-%02d %02d:%02d:%02d [%s] ",
                  tmv.tm_year + 1900, tmv.tm_mon + 1, tmv.tm_mday,
                  tmv.tm_hour, tmv.tm_min, tmv.tm_sec, lvl_str(lvl));
    const char* short_file = strrchr(file, '/');
    const char* short_file2 = strrchr(file, '\\');
    if (short_file2 > short_file) short_file = short_file2;
    if (!short_file) short_file = file; else short_file++;
