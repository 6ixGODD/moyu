...
char* context_to_json(const context_store* s, size_t last_n) {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    int hn = snprintf(hdr,
                      ...
                      (unsigned long long)nd->id,
                      type_str,
                      (unsigned long long)nd->ts_ms);
    for (int k = 0; k < hn; k++)
      putc_buf(&buf, &len, &cap, hdr[k]);
    putc_buf(&buf, &len, &cap, '"');
    ...
    if (nd->output) {
      ...
      putc_buf(&buf, &len, &cap, 'n');
      putc_buf(&buf, &len, &cap, 'u');
      putc_buf(&buf, &len, &cap, 'l');
      putc_buf(&buf, &len, &cap, 'l');
    }
    putc_buf(&buf, &len, &cap, '}');
  }
