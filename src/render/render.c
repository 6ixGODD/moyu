...
void render_clear(render_ctx* r, uint32_t color) {
    for (int i = 0; i < r->w * r->h; i++) r->buf[i] = color;
}

void render_blit_frame(render_ctx* r, const sprite_sheet* s, int frame, int dx, int dy) {
    ...
    for (int y = 0; y < s->frame_h; y++) {
        ...
        for (int x = 0; x < s->frame_w; x++) {
            ...
            if (a == 255) {
                ...
                uint8_t dr = (dst >> 24) & 0xff, dg = (dst >> 16) & 0xff, db = (dst >> 8) & 0xff, da = dst & 0xff;
                ...
                r->buf[ty * r->w + tx] = ((uint32_t)orr << 24) | ((uint32_t)og << 16) | ((uint32_t)ob << 8) | oa;
            }
        }
    }
}

void render_blit_frame_alpha(render_ctx* r, const sprite_sheet* s, int frame, int dx, int dy, uint8_t alpha_mul) {
    if (!s || frame < 0 || frame >= s->frame_count) return;
    const uint32_t* src = sprite_frame(s, frame);
    for (int y = 0; y < s->frame_h; y++) {
        int ty = dy + y;
        ...
        for (int x = 0; x < s->frame_w; x++) {
            ...
            if (a == 0) continue;
            uint8_t sr = (p >> 24) & 0xff, sg = (p >> 16) & 0xff, sb = (p >> 8) & 0xff;
            uint32_t pp = ((uint32_t)sr << 24) | ((uint32_t)sg << 16) | ((uint32_t)sb << 8) | a;
...
void render_blit_frame_scaled(render_ctx* r, const sprite_sheet* s, int frame,
                              int dx, int dy, int scale) {
    if (!s || frame < 0 || frame >= s->frame_count || scale < 1) return;
    if (scale == 1) { render_blit_frame(r, s, frame, dx, dy); return; }
    const uint32_t* src = sprite_frame(s, frame);
    for (int y = 0; y < s->frame_h; y++) {
        for (int x = 0; x < s->frame_w; x++) {
            uint32_t p = src[y * s->frame_w + x];
