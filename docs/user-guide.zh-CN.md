...
双击桌宠会在它旁边打开一个像素风快捷输入框。输入框会限制在当前显示器工作区内，默认只有一行，随内容自动长到五行，更多内容改为文本区内部滚动。文本区保留系统原生输入法、选择、撤销和剪贴板行为，点击圆形勾选按钮发送。
...
Windows 系统托盘小图标使用跟随系统明暗主题的原生右键菜单，只保留系统级操作：打开终端聊天、打开收藏和退出。右键桌宠本体会打开原生状态窗口，里面只有语义明确的 **Talk to MOYU...** 和 **Open collected items**。普通气泡和反馈卡片不会抢键盘焦点，也不会同时叠出两套文本。

MOYU 内置 20 类程序化动作。调度器会结合好奇心、耐心、毒舌程度、愉悦度和唤醒度，在漫步、偷偷靠近指针、偷看、百无聊赖、调皮闪身、伸懒腰和打哈欠之间选择。指针靠得太近会把它赶走；性格调皮时，它可能不会害怕，而是逗你一下再闪开。
