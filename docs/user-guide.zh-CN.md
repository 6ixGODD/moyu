...
Windows 系统托盘小图标使用原生右键菜单，负责系统级操作：打开聊天、打开收藏、打开 `~/.moyu`、查看 Runtime 状态、暂停/恢复自治行为，以及退出。右键桌宠本体则会打开 MOYU 自己的 companion panel，而不是只弹一个普通菜单；这个面板会展示情绪、最近收藏、当前念头和快捷操作。普通气泡和反馈卡片不会抢键盘焦点。
